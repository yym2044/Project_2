...
<body>
	...
	<div class="container-fluid">
		...
		<div class="width65">
			<div class="row p-3">
				<div class="col">
					'<span class="fw-bold"><c:out value="${vo.shBar}"/></span>'에 대한 검색결과
				</div>
			</div>
		<div class="table-responsive">
			<table class="table">
				<c:forEach items="${list}" var="item" varStatus="status">
					<tr>
						<td><c:out value="${item.trpdSeq}"/></td>
						<td><c:out value="${item.trpdName }"/></td>
						<td><c:out value="${item.trpdPrice }"/></td>
						<td><c:out value="${item.trpdDiscountPercent }"/></td>
						<td><c:out value="${item.trpdDeliveryFee }"/></td>
						<td><c:out value="${item.trpdStarRating }"/></td>
					</tr>
				</c:forEach>
			</table>
		</div>
